...
namespace ShoeStore.Back_End
{
    public partial class Color : System.Web.UI.Page
    {
        ...
        {
            DataSet ds = ShoeStore.Data.Product.ProductsColor.GetColors();

            GridView1.DataSource = ds;
            GridView1.DataMember = "ColorType";
           
        }

        protected void Button1_Click(object sender, EventArgs e)
        {              
            Response.Redirect("~/Back_End/Color.aspx");
        }
        ...
        protected void Unnamed1_Click(object sender, EventArgs e)
        {
            ...
            {
                string lColor = /*Regex.Replace(*/tbxValue.Text;/*, @"\W", "");*/             
                ShoeStore.Data.Product.ProductsColor.AddColors(lColor);
        ...
        {  
            int lColorID = (int)GridView1.DataKeys[e.RowIndex].Value;
            int lResult = 1;

            if (lColorID > 0)
            {
               lResult =  ShoeStore.Data.Product.ProductsColor.DeleteColors(lColorID);
            }
            if (lResult==1)
            {
                lblErrorDB.Text = "В процесе выполнения произошла ошибка. Повторите попытку позже";
        ...
        protected void GridView1_RowUpdating(object sender, GridViewUpdateEventArgs e)
        {
            ...
            if (e.NewValues["ColorName"] != null)
            {
                string lcolorname = e.NewValues["ColorName"].ToString();


                lcolorname = Regex.Replace(e.NewValues["ColorName"].ToString(), @"\W", "");
                bool lResult = ShoeStore.Data.Product.ProductsColor.EditColors(lID, lcolorname);
